...
\titulo{Sudo Loja: Um Sistema de Gerenciamento de Conteúdo para Vendas na Internet}
\autor{João Marcos Ferreira} \nome{João Marcos} \ultimonome{Ferreira}
%
%---------- Informe o Curso e Grau -----
\tecnologo \curso{Tecnologia em Análise e Desenvolvimento de Sistemas} \ano{2015}
...
% O mercado da região desconhece esse tipo de ferramenta, onde poderia sem muitos gastos, expandir o seu negócio através de vendas na Internet. Em raras ocasiões, os comerciantes interessados em vender na Internet, procuram por empresas especializadas em desenvolvimento de aplicações comerciais para desenvolver seu site de vendas. O problema é que esse processo pode demorar muito, além de ser caro e muitos não têm capital o suficiente para cobrir o custo do desenvolvimento de uma aplicação desse nível ou não dão valor ao trabalho e simplesmente não querem pagar por isso.

% Além desses, um outro problema é a frequente solicitação do cliente a empresa contratada para realizar modificações, mais especificamente no \textit{layout} do sistema. Esse tipo de modificação quase sempre é demorada e, principalmente, cara, dependendo de seu tamanho.

% O sistema em desenvolvimento visa resolver esses problemas, oferecendo um CMS (do inglês \textit{Content Management System}) capaz de criar uma loja virtual personalizável.  O CMS proposto busca facilitar a criação, personalização e gestão de uma loja virtual, promovendo uma entrada rápida ao mercado virtual. Podem ser vistos vários sistemas que funcionam dessa maneira, como por exemplo: a \citeonline{tray} e \citeonline{lojaintegrada}. Na Seção \ref{cha:ferramentas_relacionadas} será apresentado um comparativo entre estas e outras ferramentas.
...
% Esse trabalho tem como objetivo principal a criação de um sistema computacional web para criação de lojas virtuais, onde um usuário cadastrado poderá criar sua loja, podendo personalizar seu \textit{layout} e disponibilizar produtos, onde clientes cadastrados nessa loja, poderão comprá-los.
...
% \begin{itemize}
% \item Analisar sistemas similares, no intuito de entender como funcionam.
% \item Disponibilizar um módulo para criação de lojas virtuais.
% \item Desenvolver um módulo para personalização.
% \item Criar um módulo para gerenciamento de vendas.
% \end{itemize}
...
% A Seção \ref{cha:fundamentaco_teorica} apresenta a fundamentação teórica para o desenvolvimento deste trabalho. Na Seção \ref{cha:metodologia} é descrita a metodologia adotada para execução do projeto. A Seção \ref{cha:ferramentas_relacionadas} apresenta ferramentas relacionadas a este projeto e um comparativo entre elas. A Seção \ref{cha:sudoloja} descreve a análise, a implementação, o projeto arquitetural e a validação. E por fim, a Seção \ref{cha:consideracoes_finais} apresenta as considerações finais do documento.
...
% \section{CMS} % (fold)
% \label{sec:cms}
% CMS é a sigla para \textit{Content Management System}, que se traduz diretamente ao português como Sistema de Gerenciamento de Conteúdos. Em linhas gerais, um CMS permitiria administrar conteúdos em meio digital e para o caso particular que nos ocupa, um CMS permitiria gerenciar os conteúdos de uma loja. \cite{miguel}

% Em outras palavras, um CMS é uma ferramenta que permite a um editor criar, e publicar qualquer tipo de informação em uma página web. Geralmente, os CMS trabalham manipulando um banco de dados, de modo que o editor simplesmente atualiza este banco, incluindo nova informação ou editando a existente.

% Contudo, o mais interessante, é que essa ferramenta é feita de tal maneira que mesmo aqueles que nunca ouviram falar de ASP, PHP, MySQL, Apache, HTML, Javascript ou qualquer outra linguagem voltada para web poderá usá-lo, inclusive esta é sua principal função, tornar acessível a todos a sua presença na Internet através de um site, facilitando a inserção de textos, de comentários e dezenas de outras funcionalidades, de acordo com as características de cada aplicação. 

% \textit{Podemos dizer que um CMS é um software que pega uma estrutura pré moldada e transforma em um web site com base nos comandos passados pelo administrador} \cite{raven}. Alguns Exemplos de CMS podem ser vistos a seguir.

% \begin{itemize}
% \item \textit{Drupal} - É uma plataforma de gerenciamento de conteúdo de código aberto alimentando milhões de web sites e aplicações. Ele é construído, usado, e apoiado por uma comunidade ativa e diversificada de pessoas ao redor do mundo \cite{drupal}.
...
% \item \textit{Wordpress} - Sistema muito popular e bastante usado pelos \textit{bloggers}. WordPress é um software web que você pode usar para criar um belo site, blog, ou app \cite{wordpress}.
% \end{itemize}
...
% \textit{e-commerce}, também chamado de comércio eletrônico, é aplicável a qualquer tipo de transação comercial que pode envolver compra, venda, transferência ou troca de produtos, serviços ou informações por meio de redes de computadores, incluindo
% a Internet \cite{turban}. Entretanto, existem diferenças entre \textit{e-commerce} e e-business. E-business é definido como o uso das tecnologias da informação para executar funções de negócios. E-business é, portanto, um termo mais amplo que inclui
% \textit{e-commerce} \cite{gordon}. Segundo \citeonline{fernandes} \textit{e-commerce} é dividido nos seguintes tipos:
...
% \item \textbf{Consumidor para Empresa (C2B – Consumer to Business):} O sentido tradicional da troca dos bens é invertido. Acontece quando um número de indivíduos coloca os seus bens ou serviços para serem adquirido por empresas que procuram por tal bem ou serviço;
% \item \textbf{Empresa para Administração (B2A – Business to Administration):} Para esse tipo de comércio são consideradas todas as transações que ocorrem entre empresas e a administração pública;
% \item \textbf{Consumidor para Administração (C2A – Consumer to Administration):} Ocorre nas transações entre indivíduos e a administração pública.
% \end{itemize}

% O sistema que será apresentado se encaixa no tipo B2C (\textit{Business to Consumer}) onde o Lojista fará o negócio diretamente com os consumidores finais.
...
% \begin{figure}[H]
% \centering
% \includegraphics[width=15cm]{img/prototipos/produtos-cadastrados.eps}
...
% \begin{figure}[H]
% \centering
% \includegraphics[width=15cm]{img/prototipos/cadastro-produto.eps}
...
% \begin{figure}[H]
% \centering
% \includegraphics[width=15cm]{img/prototipos/visualizacao-pedido.eps}
...
% \begin{figure}[H]
% \centering
% \includegraphics[width=15cm]{img/prototipos/painel-personalizacao.eps}
...
% \begin{figure}[H]
% \centering
% \includegraphics[width=15cm]{img/prototipos/estoque.eps}
...
% \begin{figure}[H]
% \centering
% \includegraphics[width=15cm]{img/prototipos/visualizacao-gs.eps}
