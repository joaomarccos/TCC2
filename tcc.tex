...
A realização dos estudos deste trabalho segue a mesma metodologia de trabalhos científicos. "Cientifico não é considerado como algo pronto, acabado ou definitivo” \cite{sa2010anuario}, isso permite que os contextos existentes sejam interpretados e discutidos de diferentes formas por diferentes pessoas.
...
\section{Etapas do processo} % (fold)
\label{sec:atividades}
...
Esta seção irá discutir as etapas de análise, projeto e implementação d Sudo Loja. Na seção \ref{sec:ferramentas_relacionadas}, uma análise das ferramentas relacionadas foi levantada.
...
A seguir serão apresentados os artefatos produzidos na fase de análise, inerentes a algumas \textit{User Stories} específicas. Para isso foram escolhidas as \textit{User Stories} US03 (O sistema deve realizar o gerenciamento do estoque da loja virtual) e US10 (O sistema deve concretizar vendas através de diferentes meios de pagamento).
...
\citeonline{booch2006uml} nos dizem que esses aspectos podem ser considerados uma representação de seu esqueleto e estrutura relativamente estáveis. O diagrama de componentes por ser um diagrama estrutural, é útil para representarmos a arquitetura de um sistema.
...
O sistema se encontra \textit{online} e pode ser acessado a partir das URLs \url{https://sudoloja.herokuapp.com} e \url{http://sudoloja.tk}.
...
\begin{itemize}
	\item O sistema ultilizou apenas uma estratégia para a realização de pagamentos. O uso do PagSeguro. Outras plataformas poderiam ser integradas ao sistema.
	\item A personalização foi feita apartir de um template pré definido. O sistema poderia disponibilizar a escolha dentre diversos templates.
	\item O sistema poderia possibilitar o uso de cupoms de desconto em compras.
...
\chapter{Modelo lógigo de dados} % (fold)
